...
REPORT ZLEAD_SQL2.
 TYPES:
    ...
    SELECT crm_jest~objnr
     ...
     "INNER JOIN cgpl_project ON crm_jest~objnr = cgpl_project~guid
    ...
    OPEN CURSOR WITH HOLD @lv_cursor FOR
      SELECT DISTINCT header~guid,
                      ...
                      text1,
                      header~object_type
                 FROM cgpl_project AS header" where object_type = 'CPG'
    ...
    DO.
      ...
      lv_table_size = lines( et_campaigns ).
      IF  lv_table_size >= lv_max_hits.
          EXIT.
      ELSE.
          lv_package_size = lv_max_hits - lv_table_size.
      ENDIF.

    ENDDO.
    CLOSE CURSOR @lv_cursor.

    WRITE:/ 'Total numbers of leads read: ', lines( et_campaigns ), ' time: ' , zcl_jerry_tool=>end_measure( ).