FUNCTION ZCRM_ORDERADM_I_SELECT_S_DB .
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
*"     REFERENCE(IV_GUID) TYPE  CRMT_OBJECT_GUID
*"  EXPORTING
*"     REFERENCE(ES_ORDERADM_I_DB) TYPE  CRMD_ORDERADM_I
*"  EXCEPTIONS
*"      ITEM_DOES_NOT_EXIST
*"----------------------------------------------------------------------
...
DATA: lv_dummy_message(1)   TYPE   c,
  ...
  CLEAR es_orderadm_i_db.

  IF iv_guid IS INITIAL.
    MESSAGE i202(crm_order_misc) RAISING parameter_error.
  ENDIF.

  SELECT SINGLE * INTO ls_btx_i FROM zcrms4d_btx_i WHERE item_guid = iv_guid.
  IF sy-subrc NE 0.
...
*  Jerry: Ugly!!
     IF <lv_object> = 'ORDERADM_I'.
        es_orderadm_i_db = <ls_wrk_structure>.
     ENDIF.
