...
WHILE ip_instance IS NOT INITIAL.
  ...
  DATA(item_handler) = cl_crm_ipm_item_handler_provid=>get_instance( ip_instance ).

  DATA(wrapped_ip) = cl_crm_ipm_order_item_entity=>get_instance( io_original = ip_instance iv_ipm_admini_type = 'IP' ).
...
ENDWHILE.
  ...
  WRITE: / 'IP Scope generated successfully!'.